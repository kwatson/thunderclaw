...
async function main() {
  ...
  try {
    ...
    assert(status.gatewayVersion === "2026.9.1" && status.capabilities?.flatListItemReplacement === true
    ...
    const gatewayImageId = command("docker", ["image", "inspect", "ghcr.io/openclaw/openclaw:2026.9.1", "--format", "{{.Id}}"], { capture: true });
    ...
    const gatewayRepoDigests = command("docker", ["image", "inspect", "ghcr.io/openclaw/openclaw:2026.9.1", "--format", "{{json .RepoDigests}}"], { capture: true });
