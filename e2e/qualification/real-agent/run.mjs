...
async function waitForOperatorGateway() {
  for (let attempt = 0; attempt < 30; attempt += 1) {
    const result = spawnSync("docker", ["compose", "-f", "compose.spike.yaml", "exec", "-T", "gateway",
      "node", "openclaw.mjs", "gateway", "call", "health", "--json"], { cwd: root, stdio: "ignore" });
    if (result.status === 0) return;
    await new Promise((done) => setTimeout(done, 1000));
  }
  throw new Error("Gateway operator API did not become ready");
}
...
async function main() {
  ...
  try {
    ...
    assert(status.gatewayVersion === "2026.9.2" && status.capabilities?.flatListItemReplacement === true
    ...
    const gatewayImageId = command("docker", ["image", "inspect", "ghcr.io/openclaw/openclaw:2026.9.2", "--format", "{{.Id}}"], { capture: true });
    ...
    const gatewayRepoDigests = command("docker", ["image", "inspect", "ghcr.io/openclaw/openclaw:2026.9.2", "--format", "{{json .RepoDigests}}"], { capture: true });
