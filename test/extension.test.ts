...
test("ClawHub catalog metadata presents the ThunderClaw brand", async () => {
  ...
  assert.equal(pluginPackage.peerDependencies.openclaw, ">=2026.7.2-beta.7 <2026.9.5-0");
  assert.equal(pluginPackage.openclaw.compat.pluginApi, ">=2026.7.2-beta.7 <2026.9.5-0");
...
test("Thunderbird extension declares compose and message-view boundaries", async () => {
  ...
  assert.equal(pluginPackage.version, "0.1.8");
