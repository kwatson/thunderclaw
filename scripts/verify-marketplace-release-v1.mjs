// Frozen verifier for the legacy shared v0.1.0 and v0.1.1 releases.
// Do not broaden this parser or schema; new component releases use v2.
...
const allowedLegacyTags = new Set(["v0.1.0", "v0.1.1"]);

export function expectedArtifactNamesV1(version) {
  return [
...
export function parseChecksumsV1(contents) {
...
export async function verifyMarketplaceReleaseV1({ directory, tag, repository, commit }) {
  if (!releaseTagPattern.test(tag) || !allowedLegacyTags.has(tag)) {
    throw new Error(`Legacy release tag must be v0.1.0 or v0.1.1: ${tag}`);
  }
  if (!/^[a-f0-9]{40}$/u.test(commit)) throw new Error(`Release commit must be a full SHA-1: ${commit}`);
  const version = tag.slice(1);
  const expectedNames = expectedArtifactNamesV1(version);
  ...
  const checksums = parseChecksumsV1(await readFile(path.join(directory, "SHA256SUMS"), "utf8"));
  ...
  const byName = new Map(provenance.artifacts.map((artifact) => [artifact?.name, artifact]));
  ...
  return { tag, version, commit, artifacts };
}
...
function parseArguments(argumentsList) {
  ...
  for (let index = 0; index < argumentsList.length; index += 2) {
    ...
    if (!["--directory", "--tag", "--repository", "--commit"].includes(key) || !value || values.has(key)) {
      throw new Error("Usage: verify-marketplace-release-v1.mjs --directory <dir> --tag v0.1.<0|1> --repository owner/repo --commit <sha>");
    }
    values.set(key, value);
  }
  if (values.size !== 4) throw new Error("All legacy release verification arguments are required");
  return Object.fromEntries([...values].map(([key, value]) => [key.slice(2), value]));
}

if (process.argv[1] && pathToFileURL(path.resolve(process.argv[1])).href === import.meta.url) {
  try {
    const result = await verifyMarketplaceReleaseV1(parseArguments(process.argv.slice(2)));
